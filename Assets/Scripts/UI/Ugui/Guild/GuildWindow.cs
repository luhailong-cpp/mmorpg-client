...
namespace MmorpgClient.UI.Ugui.Guild
{
    ...
    public sealed class GuildWindow
    {
        ...
        public GuildWindow(UnityEngine.Transform parent)
        {
            ...
            Art(_frame, "main_frame", 0, 0, 2160, 924);
            Art(_frame, "title_plate", 62, -21, 480, 114);
            Text(_frame, "同道成一家", 100, -7, 400, 84, 47, Cream, alignment: TextAlignmentOptions.Center);
            _summary = Text(_frame, "结一方同道 · 守一盏人间灯", 82, 106, 1800, 46, 29, Muted);
            string[] titles = { "帮会", "成员", "排行", "捐献", "活动", "商店" };
            for (int i = 0; i < _tabs.Length; i++)
            {
                GuildPage page = (GuildPage)i;
                _tabs[i] = NamedButton(_frame, "GuildTab_" + page, titles[i],
                    640 + i * 226, 31, 210, 76, () => Show(page), key: "tab_normal", fontSize: 30);
            }
            NamedButton(_frame, "CloseGuild", "", 2036, 28, 76, 76, Hide, key: "close");
            _rail = QdaoUguiFactory.CreateRect("GuildIdentity", _frame, 60, 172, 480, 640);
            _body = QdaoUguiFactory.CreateRect("GuildContent", _frame, 562, 172, 1536, 640);
            _status = Text(_frame, "", 84, 828, 1700, 55, 27, Muted, true);
            _refresh = NamedButton(_frame, "RefreshGuild", "刷新", 1890, 828, 180, 64,
        ...
        private void Render()
        {
            ...
            _summary.text = _client?.Info == null ? "结一方同道 · 守一盏人间灯" :
                _client.Info.Name + "  ·  " + _client.Info.Members.Count + " / " + _client.Info.MaxMembers + " 位同道";
            _refresh.interactable = !Busy;
            for (int i = 0; i < _tabs.Length; i++)
            {
                _tabs[i].image.sprite = Load(i == (int)Page ? "tab_selected" : "tab_normal");
        ...
        {
            Art(_rail, "content_panel", 0, 0, 480, 640);
            QdaoUguiFactory.CreateImage("GuildCrest", _rail, 138, 20, 204, 204, GuildSprite("crest")).preserveAspect = true;
            var info = _client?.Info;
            Text(_rail, info?.Name ?? "静候同道", 32, 225, 416, 64, 40, alignment: TextAlignmentOptions.Center);
            if (info == null)
            {
                Text(_rail, "灯火可亲，同道可期。\n找到一处归属，携手闯荡山海。", 44, 312, 392, 140, 29, Muted, true);
                NamedButton(_rail, "BrowseGuilds", "寻找帮会", 70, 472, 340, 76, () => Show(GuildPage.Ranking), true);
                return;
            }
            Text(_rail, "帮会等级", 44, 316, 230, 48, 28, Muted);
            Text(_rail, "Lv." + info.Level, 290, 316, 140, 48, 34, Gold, alignment: TextAlignmentOptions.MidlineRight);
            Text(_rail, "帮会编号", 44, 375, 392, 36, 24, Muted);
            Text(_rail, info.GuildId.ToString(), 44, 414, 392, 42, 25);
            Text(_rail, "我的身份", 44, 478, 230, 48, 28, Muted);
            Text(_rail, RoleName(_client.Role), 290, 478, 140, 48, 30, alignment: TextAlignmentOptions.MidlineRight);
            NamedButton(_rail, "GuildMembersEntry", "查看同道", 70, 548, 340, 70, () => Show(GuildPage.Members));
        }
        private void RenderOverview()
        {
            var info = _client?.Info;
            Art(_body, "content_panel", 0, 0, 1536, 640);
            if (info == null)
            {
                Text(_body, "一盏灯火，一处归属", 100, 64, 1350, 74, 49);
                Text(_body, _client?.HasLoaded == true ? "你尚未加入帮会。可以浏览帮会排行，或创建自己的帮会。"
                    : _client?.RequiresReconnect == true ? GuildClient.RecoveryMessage : "正在等候帮会信息。若服务暂未响应，可点击右下角刷新。", 100, 174, 1350, 110, 31, Muted, true);
                NamedButton(_body, "OpenGuildRanking", "寻找帮会", 100, 350, 350, 88, () => Show(GuildPage.Ranking), true);
                NamedButton(_body, "OpenCreateGuild", "创建帮会", 488, 350, 350, 88, ShowCreate,
                    enabled: _client?.HasLoaded == true && !Busy);
                Text(_body, "成员信息、公告和排行将随帮会更新。", 100, 516, 1320, 55, 28, Muted);
                return;
            }
            Text(_body, "帮会公告", 100, 35, 790, 70, 42);
            NamedButton(_body, "ReadGuildAnnouncement", "查看全文", 934, 37, 260, 68, ShowReadAnnouncement, fontSize: 28);
            if (_client.CanEditAnnouncement)
                NamedButton(_body, "EditGuildAnnouncement", "编辑公告", 1220, 37, 270, 68, ShowAnnouncement, enabled: !Busy, fontSize: 28);
            Text(_body, string.IsNullOrWhiteSpace(info.Announcement) ? "帮会尚未发布公告。愿同道相伴，诸事顺遂。" : info.Announcement,
                100, 138, 1330, 300, 33, wrap: true);
            Text(_body, "帮主 · " + info.LeaderId, 100, 490, 810, 52, 28, Muted);
            Text(_body, "一同修行，一同守护这方灯火。", 100, 552, 1000, 50, 28, Muted);
            NamedButton(_body, "LeaveOrDisbandGuild", _client.IsLeader ? "解散帮会" : "退出帮会", 1210, 534, 280, 76,
                () => Confirm(_client.IsLeader ? "确认解散帮会" : "确认退出帮会",
                    _client.IsLeader ? "解散后所有成员都将离开帮会，此操作无法恢复。" : "退出后将失去当前帮会身份。确认要离开同道吗？",
                    () => { if (_client.IsLeader) DisbandRequested?.Invoke(); else LeaveRequested?.Invoke(); }), enabled: !Busy, fontSize: 29);
        }
        private void RenderMembers()
        {
            ...
            var search = Input(_body, "GuildMemberSearch", 640, 0, 600, 66, "输入完整编号或部分数字", 20);
            search.SetTextWithoutNotify(_memberSearch);
            NamedButton(_body, "SearchGuildMembers", "查找", 1260, 0, 268, 66,
            ...
            for (int i = 0; i < MembersPerPage && _memberPage * MembersPerPage + i < members.Count; i++)
            {
                ...
                float y = 88 + i * 90;
                GuildField(_body, "GuildListRow", 0, y, 1536, 82);
                ...
                Text(_body, member.Online ? "在线" : "离线", 1330, y + 12, 170, 56, 28, member.Online ? Ink : Muted);
            }
        ...
        private void RenderRanking()
        {
            ...
            Text(_body, "同道云集 · 帮会排行", 16, 0, 1440, 68, 39);
            ...
            if (rank != null)
                for (int i = 0; i < rank.Entries.Count && i < 5; i++)
                {
                    ...
                    NamedButton(_body, "JoinGuild_" + id, _client?.Info?.GuildId == id ? "我的帮会" : "加入",
                        1290, y + 9, 215, 64, () => Confirm("确认加入帮会", "帮会名称：" + name + "\n确认加入此帮会，与同道共赴山海？", () => JoinRequested?.Invoke(id)),
        ...
        {
            Text(_body, title, 12, 0, 1450, 64, 42);
            Text(_body, subtitle, 12, 80, 1450, 58, 30, Muted);
            for (int i = 0; i < 3; i++)
            {
                float x = i * 512;
                Art(_body, "content_panel", x, 172, 492, 410);
                ...
                GuildIcon(_body, icons[i], x + 190, 194, 112);
                Text(_body, titles[i], x + 30, 305, 432, 66, 36, alignment: TextAlignmentOptions.Center);
                Text(_body, descriptions[i], x + 40, 388, 412, 94, 28, Muted, true);
                NamedButton(_body, "GuildUnavailable_" + Page + "_" + i, "暂未开放", x + 70, 504, 352, 64, null,
                    enabled: false, fontSize: 28);
            }
            Text(_body, "开放后可在此查看规则与参与条件。", 14, 591, 1480, 44, 27, Muted);
        }
        private void Pager(UnityEngine.Transform parent, string prefix, int page, int pages, Action<int> changed, bool enabled)
        {
            NamedButton(parent, prefix + "Previous", "上一页", 854, 568, 210, 64, () => changed(-1), enabled: enabled && page > 1, fontSize: 27);
            Text(parent, page + " / " + pages, 1078, 568, 212, 64, 29, Muted, alignment: TextAlignmentOptions.Center);
            NamedButton(parent, prefix + "Next", "下一页", 1300, 568, 210, 64, () => changed(1), enabled: enabled && page < pages, fontSize: 27);
        }
        private void ShowCreate()
        {
            if (Busy || _client?.HasLoaded != true || _client.Info != null) return;
            var frame = Modal("创建帮会", "为同道起一个名字，点亮属于你们的灯火。");
            Text(frame, "帮会名称", 66, 212, 970, 54, 30);
            var input = Input(frame, "NewGuildName", 66, 280, 968, 82, "1–24 字帮会名称", 24);
        ...
        private void ShowAnnouncement()
        {
            ...
            var frame = Modal("编辑帮会公告", "帮主与长老可编辑公告，最多 500 字。");
            Text(frame, "公告内容", 66, 192, 968, 48, 29);
            var input = Input(frame, "GuildAnnouncementInput", 66, 247, 968, 176, "写下给同道的话", 500);
            input.lineType = TMP_InputField.LineType.MultiLineNewline;
            input.textComponent.textWrappingMode = TextWrappingModes.Normal;
            input.textComponent.alignment = TextAlignmentOptions.TopLeft;
        ...
        private RectTransform Modal(string title, string description)
        {
            ...
            Art(frame, "main_frame", 0, 0, 1100, 610);
            Text(frame, title, 66, 50, 960, 72, 42);
        ...
        private static TMP_InputField Input(UnityEngine.Transform parent, string name, float x, float y, float w, float h, string placeholder, int max)
        {
            ...
            input.textComponent.fontSize = 29; input.textComponent.richText = false;
            if (input.placeholder is TMP_Text hint) { hint.fontSize = 26; hint.richText = false; }
            return input;
        }
